...
def get_wallet(bot_addr):
    ...
    bot_wallet_query = wallets_ref.order_by_child(
        K_UID).equal_to(hex_uid).limit_to_first(1)
...
def fund_adjudicator(contract_addr, bot_addr):
    ...
    eth_nonce = o_w3.eth.getTransactionCount(
        from_addr)
    ...
    _, wallet_key = get_wallet(bot_addr)
    nonce = get_wallet_ref(wallet_key).child(
        K_NONCE).transaction(increment_nonce)
    ...
    signed = o_w3.eth.account.signTransaction(
        transaction, private_k)  # pylint: disable=E1101
    _sent_tx = o_w3.eth.sendRawTransaction(
        signed.rawTransaction)  # pylint: disable=E1101
